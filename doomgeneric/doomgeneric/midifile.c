...
// Non-SDL big-endian byteswap shims (host is little-endian aarch64).
#include <stdint.h>
static inline uint32_t _be32(uint32_t x) {
    return (x >> 24) | ((x >> 8) & 0xff00) | ((x << 8) & 0xff0000) | (x << 24);
}
static inline uint16_t _be16(uint16_t x) { return (uint16_t)((x >> 8) | (x << 8)); }
#define SDL_SwapBE32(x) _be32(x)
#define SDL_SwapBE16(x) _be16(x)

// Helpers newer Chocolate Doom expects but doomgeneric lacks:
#define M_fopen(name, mode) fopen((name), (mode))
void *I_Realloc(void *ptr, size_t size)
{
    void *p = realloc(ptr, size);
    if (size != 0 && p == NULL)
    {
        fprintf(stderr, "I_Realloc: failed to reallocate %zu bytes\n", size);
        exit(1);
    }
    return p;
}
