...
extern opl_driver_t opl_unifi_driver;
...
// The callback that signals us fires from the render thread as it advances time;
// a timeout guards against init hanging if audio isn't being consumed.
...
void OPL_Delay(uint64_t us)
{
    ...
    pthread_mutex_init(&delay_data.mutex, NULL);
    pthread_cond_init(&delay_data.cond, NULL);

    OPL_SetCallback(us, DelayCallback, &delay_data);

    clock_gettime(CLOCK_REALTIME, &deadline);
    deadline.tv_sec += 2;   // safety timeout

    pthread_mutex_lock(&delay_data.mutex);
    while (!delay_data.finished)
    {
        if (pthread_cond_timedwait(&delay_data.cond, &delay_data.mutex,
                                   &deadline) != 0)
        {
            break;          // timed out; give up waiting
        }
    }
    pthread_mutex_unlock(&delay_data.mutex);

    pthread_mutex_destroy(&delay_data.mutex);
    pthread_cond_destroy(&delay_data.cond);
}
